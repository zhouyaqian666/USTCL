  ...
  def forward(self, x):
      ...
      for v in x:
          ...
          if self.use_bottleneck and self.new_cls:
              v = self.bottleneck(v)
          if self.cos_dist:
              x_norm = torch.norm(v, p=2, dim=1).unsqueeze(1).expand_as(v)
              v = v.div(x_norm + 0.00001)
          view_pool.append(v)
      sum_view = view_pool[0]
      length = len(view_pool)
      for index in range(1, length, 1):
          sum_view = torch.max(sum_view, view_pool[index])
      y = self.fc(sum_view)
      return sum_view, y, M
...
# For SVHN dataset
class SVHN_EnsembNet(nn.Module):
    def __init__(self):
        super().__init__()
...
class USPS_EnsembNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv_params = nn.Sequential(
        ...
        self.fc_params = nn.Sequential(nn.Linear(64*4*4, 256), nn.ReLU())
